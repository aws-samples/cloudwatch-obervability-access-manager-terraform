...
2. Remove the `OAM Link` module call from `main.tf` as post point-1 above `OAM Sink` is setup already. This step is about setup and connect the `OAM Link` from each of the vended accounts by AFT to Sink. The `OAM Link` connection with `OAM Sink` module is done using the `sink_arn` parameter `(deployments/aft-account-customizations/LOGGING/terraform/main.tf )` of the `OAM Link` module (which was installed in previous step). 
